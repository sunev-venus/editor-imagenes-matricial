...
function obtenerDimensionesImagen(rutaImagen) {
    const buffer = fs.readFileSync(rutaImagen); // Lee el contenido binario del archivo
    const png = PNG.sync.read(buffer); // Decodifica el buffer para obtener el objeto PNG

    return { // Devuelve un objeto con las dimensiones
        ancho: png.width, // Obtiene el ancho
        alto: png.height, // Obtiene el alto
        totalPixeles: png.width * png.height // Calcula y obtiene el total de píxeles
    };
}
...
function invertirColores(matriz) {
  const resultado = copiarMatriz(matriz); // crea una copia

  for (let i = 0; i < resultado.length; i++) { // i=fila j=columna
    for (let j = 0; j < resultado[i].length; j++) {
      resultado[i][j] = { // actualiza el píxel 
        // lógica de inversión negativo
        r: 255 - matriz[i][j].r,
        g: 255 - matriz[i][j].g,
        b: 255 - matriz[i][j].b,
        a: matriz[i][j].a, // canal Alpha
      };
    }
  }
...
function convertirEscalaGrises(matriz) {
  ...
  for (let i = 0; i < resultado.length; i++) { // filas
    for (let j = 0; j < resultado[i].length; j++) { // pixeles dentro da cada fila
      const px = matriz[i][j]; // pixel original 
      // Calcula el valor de luminacia (gris)
      const gris = limitarValorColor( 
        0.299 * px.r + 0.587 * px.g + 0.114 * px.b // fórmula
      );

      resultado[i][j] = { // Actualiza el píxel en la matriz resultado
        r: gris,
        g: gris,
        b: gris,
        a: px.a,
...
function aplicarSepia(matriz) {
  const resultado = copiarMatriz(matriz); // Hace una copia de la matriz

  for (let i = 0; i < resultado.length; i++) { // Recorre las filas
    for (let j = 0; j < resultado[i].length; j++) { // Recorre los píxeles
      const pixel = matriz[i][j]; // Píxel original

      const r = 0.393 * pixel.r + 0.769 * pixel.g + 0.189 * pixel.b; // Calcula el nuevo valor Rojo con fórmula sepia
      const g = 0.349 * pixel.r + 0.686 * pixel.g + 0.168 * pixel.b; // Calcula el nuevo valor Verde con fórmula sepia
      const b = 0.272 * pixel.r + 0.534 * pixel.g + 0.131 * pixel.b; // Calcula el nuevo valor Azul con fórmula sepia

      resultado[i][j] = { // Reemplaza el píxel
        r: limitarValorColor(r), // Asigna el nuevo Rojo (limitado)
        g: limitarValorColor(g), // Asigna el nuevo Verde (limitado)
        b: limitarValorColor(b), // Asigna el nuevo Azul (limitado)
        a: pixel.a // Mantiene el Alpha
      };
    }
  }

  return resultado; // Devuelve la imagen sepia
}
...
function detectarBordes(matriz, umbral = 50) {
  ...
  const grises = convertirEscalaGrises(matriz); // Convierte la imagen a grises
  const filas = grises.length; // Obtiene el número de filas (alto)
  const columnas = grises[0].length; // Obtiene el número de columnas (ancho)
  const resultado = crearMatrizVacia(filas, columnas); // Crea una matriz vacía para el resultado
  // Inicializa la matriz resultado en negro (fondo)
  for (let i = 0; i < filas; i++) { // Recorre todas las filas
    for (let j = 0; j < columnas; j++) { // Recorre todos los píxeles
      resultado[i][j] = { r: 0, g: 0, b: 0, a: 255 }; // Píxel inicial negro
    }
  }
  // 2. Compara píxeles adyacentes
  for (let i = 0; i < filas - 1; i++) { // Itera hasta la penúltima fila
    for (let j = 0; j < columnas - 1; j++) { // Itera hasta la penúltima columna
      const actual = grises[i][j].r; // Valor del píxel actual
      const derecha = grises[i][j + 1].r; // Valor del píxel derecho
      const abajo = grises[i + 1][j].r; // Valor del píxel inferior

      const diffDerecha = Math.abs(actual - derecha); // Calcula diferencia con la derecha
      const diffAbajo = Math.abs(actual - abajo); // Calcula diferencia con abajo

      // Si alguna diferencia supera el umbral, es un borde
      if (diffDerecha > umbral || diffAbajo > umbral) {
        resultado[i][j] = { r: 255, g: 255, b: 255, a: 255 }; // Marca el píxel como blanco (borde)
      }
    }
  }

  return resultado; // Devuelve la imagen con los bordes detectados
